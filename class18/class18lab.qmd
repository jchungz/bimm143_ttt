...
```
```{r}
time_length( today() - mdy("5-16-2003"), "years")
```
...
We need to **join** these two tab;es (subject and specimen) to make a single new "meta" table with all our metadata. We will use the `dplyr` join functions to do this: 
...
Can you facet or even just color by infance_vac? Is there some difference?
...
Q17. Do you see a clear difference in aP vs. wP response?

aP response tends to be higher in OVA antigen levels per visit, while wP response tends to be higher in PT antigen levels per visit.
